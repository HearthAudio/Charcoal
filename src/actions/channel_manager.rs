...
pub trait ChannelManager {
    async fn join_channel(&mut self, voice_channel_id: String,create_job: bool) -> Result<(), CreateJobError>;
    ...
    async fn join_channel(&mut self, voice_channel_id: String,create_job: bool) -> Result<(),CreateJobError> {
        ...
        if !create_job {
            self.bg_com_tx.send(IPCData::new_from_main(Message::DirectWorkerCommunication(DirectWorkerCommunication {
                ...
                seek_position: None,
                loop_times: None,
            }), self.tx.clone(),self.guild_id.clone())).context(FailedToSendIPCSnafu)?;
